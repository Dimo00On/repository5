...
/*осталось написать функцию разбора регулярки на операции,
  затем по разбору строить по операциям автомат,
  затем детерминировать его(написать функцию надо),
  затем пользуясь заполнением и инверсией, получить пдка для ответа,
  и затем собрать по этому пдка регулярку ответ с помощью функций(осталось написать базу + алгоритм сбора)
*/